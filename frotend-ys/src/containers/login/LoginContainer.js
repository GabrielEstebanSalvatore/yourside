...
const LoginContainer = () => {
    ...
    const registrarme = async () => {
        if(cliente.email =='' || cliente.password =='' || cliente.name =='' 
            || cliente.address ==''||  cliente.role ==''|| cliente.cell =='' )
        {
            handleModal('MensajeRegistro',true)
            setMessage("Tododos Los Campos son Necesarios")
            return
    ...
    const setShowModalLogin = () => { return (handleModal(localModal.modalView, localModal.showModal)) }
            ...
            <div className='login-container__footer'>
                {localState.userLoged === false &&
                    <div className='login-container__footerUp'>
                        <ButtonItemView title={'Ingresar'} onClick={login}  icon={<i class="fas fa-location-arrow"></i>}/>
                        <ButtonCancel title={'Cancelar'} onClick={(e) => setShowModalLogin()} />
                    </div>
                }
                {localState.userLoged === true &&
                    <div className='login-container__footerUp'>
                        <ButtonItemView title={'Registrarme'} onClick={registrarme}  />
                        <ButtonCancel title={'Cancelar'} onClick={(e) => setShowModalLogin()} />
                    </div>
                }
                {localState.userLoged === false &&
                    <div className='login-container__footerDown'>
                        <ButtonItemView title={'Registrarme'} onClick={(e) => setLocalState({ userLoged: true })} />
                    </div>
                }
                {
                    <div className='login-container__footerDown'>
                        {localState.userLoged === true &&
                            <ButtonItemView icon={<i class="fas fa-arrow-left"></i>} onClick={(e) => setLocalState({ userLoged: false })} />}
                    </div>
                }
            </div>
        </div>
